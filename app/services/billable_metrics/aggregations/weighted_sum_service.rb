  ...
  module Aggregations
    class WeightedSumService < BillableMetrics::Aggregations::BaseService
      ...
      def latest_value
        ...
        cached_aggregations = cached_aggregations.where(group_id: group.id) if group
        cached_aggregations = cached_aggregations.where(charge_filter_id: charge_filter.id) if charge_filter
        cached_aggregation = cached_aggregations.first

        return @latest_value = BigDecimal(cached_aggregation.current_aggregation) if cached_aggregation
      ...
      def grouped_latest_values
        ...
        grouped_by.each do |key|
          cached_aggregations = cached_aggregations.where('grouped_by?:key', key:)
        end

        cached_aggregations = cached_aggregations.where(group_id: group.id) if group
        cached_aggregations = cached_aggregations.where(charge_filter_id: charge_filter.id) if charge_filter

        if cached_aggregations.all.any?
          return @grouped_latest_values = cached_aggregations.map do |cached_aggregation|
            {
              groups: cached_aggregation.grouped_by,
              value: BigDecimal(cached_aggregation.current_aggregation),
            }
          end
        end
